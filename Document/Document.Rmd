...
# Creating a column to categorize countries by continents
data_TFP_long$Continent <- ifelse(data_TFP_long$Country %in% c(
  "Australia", "New Zealand"), "Oceania",
  ifelse(data_TFP_long$Country %in% c(
    "United States", "Canada"), "North America",
    ifelse(data_TFP_long$Country %in% c(
      "France", "Germany", "United Kingdom", "Italy", "Spain", "Netherlands", "Belgium", "Sweden", "Switzerland", "Greece", "Portugal", "Austria", "Norway", "Denmark", "Finland", "Ireland", "Luxembourg"), "Europe",
      ifelse(data_TFP_long$Country %in% c(
        "Japan", "Korea", "Israel"), "Asia", "Other"
      )
    )
  )
)
...
data_TFP_long$Continent <- factor(data_TFP_long$Continent)

# Plotting the data using the continent groupings
ggplot(data_TFP_long, aes(x = Year, y = value, color = Continent)) +
  geom_smooth(method = "loess") +
  scale_x_continuous(breaks = seq(1996, 2022, by = 2)) +
  labs(title = "Total Factor Productivity Trends by Continent", x = "Year", y = "Total Factor Productivity") +
